...
export type VerificationStatus = 'real' | 'fake' | 'misleading' | null;
...
export function ResultCard({ status, confidence, summary }: ResultCardProps) {
  ...
  return (
    <div className="animate-slide-up">
      <Card className={`overflow-hidden border-2 ${config.borderClass} ${config.glowClass} transition-all duration-300`}>
        <CardHeader className={`${config.bgClass} pb-4`}>
          <div className="flex items-start gap-4">
            <div className={`p-3 rounded-xl ${config.bgClass} ${config.colorClass}`}>
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <span className="text-sm text-muted-foreground">
                  Confidence Score: <span className={`font-bold ${config.colorClass}`}>{confidence}%</span>
                </span>
              </div>
            </div>
          </div>
